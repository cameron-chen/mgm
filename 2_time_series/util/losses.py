...
def compute_sinkhorn(x, y, h, M, epsilon=0.1, niter=10, scale_coef=1):
    """
...
def compute_mixed_sinkhorn_loss(f_real, f_fake, m_real, m_fake, h_fake, sinkhorn_eps, sinkhorn_l,
                                f_real_p, f_fake_p, m_real_p, h_real_p, h_fake_p, scale_coef=False):
    '''
    ...
    loss_xy = compute_sinkhorn(f_real, f_fake, h_fake, m_real, sinkhorn_eps, sinkhorn_l, scale_coef=scale_coef)
    loss_xyp = compute_sinkhorn(f_real_p, f_fake_p, h_fake_p, m_real_p, sinkhorn_eps, sinkhorn_l, scale_coef=scale_coef)
    loss_xx = compute_sinkhorn(f_real, f_real_p, h_real_p, m_real, sinkhorn_eps, sinkhorn_l, scale_coef=scale_coef)
    loss_yy = compute_sinkhorn(f_fake, f_fake_p, h_fake_p, m_fake, sinkhorn_eps, sinkhorn_l, scale_coef=scale_coef)
...
def abs_autocorr(fake_sample, real_sample):
    """Compute absolute difference of autocorrelation coefficients
       The idea is to compute the autocorrelation of each feature,
       basically compute autocorrelation of matrix (timesteps, 
       batch_size), row represents variables, column represents
       observations. Compute the autocorrelation across features.

    Args:
        fake_sampel: a 3D tensor with shape (batch_size, timesteps, features)
        real_sample: a 3D tensor with shape (batch_size, timesteps, features)

    Return:
        loss: summation of absolute difference of autocorrelation coefficients averaged 
            across batch size
    """
    acs_fake = acf_3d(fake_sample)
    acs_real = acf_3d(real_sample)

    loss = np.abs(acs_fake-acs_real)
    loss = loss.sum()

    return loss

def acf_3d(data):
    """Compute autocorrelation of a 3D tensor (batch_size, timesteps, features)

    Return:
        autocs: a list of autocorrelation coefficients across features
    """
    autocs = []
    for cha in range(data.shape[-1]):
        autoc = np.corrcoef(data[...,cha].T)[:,0]
        autocs = np.concatenate([autocs, autoc])

    return np.asarray(autocs)
