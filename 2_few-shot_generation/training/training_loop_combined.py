...
def training_loop(
    ...
    for gpu in range(num_gpus):
        with tf.name_scope('GPU%d' % gpu), tf.device('/gpu:%d' % gpu):
            ...
            G_loss_combined = SYS_args['alpha']*tf.reduce_mean(G_loss)+(1-SYS_args['alpha'])*G_loss_sys
            G_opt.register_gradients(G_loss_combined, G_gpu.trainables)
    ...
    while cur_nimg < total_kimg * 1000:
        ...
        for _repeat in range(minibatch_repeats):
            ...
            if len(rounds) == 1:
                ...
                feed_dict.update({
                    lrate_in_sys: sched.G_lrate*SYS_args['lrate_coe_sys'],
                    real_B_sys:real_B_sys_img
                })
                _, __, fake_B_sys_img = tflib.run([G_train_op, data_fetch_op,output_G_sys], feed_dict)
                if run_G_reg:
                    tflib.run(G_reg_op, feed_dict)
                # update D_sys via system loss
