...
# Resume model from the TF checkpoint
def load_ckpt(sess, checkpoint_dir, G, minibatch_size):
    ''' load checkpoint of the system. This function is model-specific

    Args:
     - checkpoint_dir: checkpoint path
     - G: agent generator
    '''
    with tf.name_scope('cyclegan'):
        latents = tf.random_normal([minibatch_size] + G.input_shapes[0][1:])
        labels = tf.zeros([minibatch_size,0])
        rho = np.array([1])
        fake_images_out, _ = G.get_output_for(latents, labels, rho, is_training=True, return_dlatents=True)
        # horse - permulate axis: (?, 3, 256, 256) -> (?, 256, 256, 3) -> manufacture (?, 256, 256, 6)
        fake_images_out_perm = tf.transpose(fake_images_out, perm=[0,2,3,1])
        real_B_sys = tf.placeholder( # placeholder, image B
            tf.float32, name='image_B_real', 
            shape=[None]+fake_images_out_perm.get_shape().as_list()[1:]
        )
        fake_images_out_perm = tf.concat([fake_images_out_perm, real_B_sys], axis=-1)
        # [horse, zebra]
    ...
    if ckpt:
        ...
        saver = tf.train.import_meta_graph(
            os.path.join(checkpoint_dir, ckpt_name+'.meta'),
            input_map = {'cyclegan/real_A_and_B_images:0': fake_images_out_perm}
        )
        saver.restore(sess, os.path.join(checkpoint_dir,ckpt_name))
        (_,                 # placeholder, input of G_sys   | D_sys
        output_G_sys,       # output of G_sys, zebra image  | D_sys
        DB_fake,            # D score of fake image         | G_sys
        fake_B_sys,         # placeholder, fake images      | D_sys 
        D_loss_sys          # D_sys loss                    | D_sys
        )=\
            tf.get_collection('key_node')
        
        return real_B_sys, output_G_sys, DB_fake, fake_B_sys, D_loss_sys
    else:
        raise Exception("No checkpoint: \
            Please download <cyclegan.zip> and unzip to <./checkpoint/system_models>")
