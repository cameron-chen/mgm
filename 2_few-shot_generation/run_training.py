...
def run(g_loss, g_loss_kwargs, d_loss, d_loss_kwargs, dataset_train, dataset_eval, data_dir, result_dir, config_id, num_gpus, total_kimg, gamma, mirror_augment, metrics, resume_pkl, resume_kimg, resume_pkl_dir, max_images, lrate_base, img_ticks, net_ticks, skip_images, 
    ...
    if rfn == 'alter':
        ...
        train = EasyDict(run_func_name='training.training_loop_combined.training_loop') # Options for training loop.
    ...
    train.hier_training = hier
    SYS['freeze_G_sysloss'] = fg
    SYS['freeze_D'] = fd
    SYS['lrate_coe_sys'] = lrc_sys
    SYS['alpha']=alpha
...
def main():
    ...
    parser.add_argument('--hier', help='train a hierarchcial model', action='store_true')
    parser.add_argument('--fd', help='number of freezed D net layers, from 0 to 7', default=0, type=int)
    parser.add_argument('--fg', help='number of freezed G net layers when L_sys backprop, from 7 to 1', default=0, type=int)
    parser.add_argument('--lrc_sys', help='coefficient of system learning rate, lr_sys = lrc_sys*lrate_in (agent lr)', default=1., type=float)
    parser.add_argument('--alpha', help='weight of combined loss, from 0 to 1', default=1, type=float)
    parser.add_argument('--rfn', help='run function name', default='alter', choices=['alter','combined'])
