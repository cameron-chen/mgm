...
def main(opt, timestamp = None, trial = None):
    ...
    if opt.dname == 'AROne':
        ...
        train_data = train_data[:,:,1].reshape(train_data.shape[0],time_steps,-1) # agent gan only generates 1-d variable data
        val_data = val_data[:,:,1].reshape(val_data.shape[0],time_steps,-1) #* used to sample val data
        ...
        val_data = data_util.real_data_loading(
            opt.dname, seq_len=time_steps,
            stride=opt.stride_val, nor_method='z_score')
        train_data = train_data[:opt.low_data_size,:,1].reshape(opt.low_data_size,time_steps,-1)
        val_data = val_data[:,:,1].reshape(val_data.shape[0],time_steps,-1) #* used to sample val data
    else:
    ...
    if bool(opt.tb_log): # tb_log: tensorboard logs
        ...
        if opt.dname == 'Elec_low':
            np.save(
                './output/{}/data/low_sample'.format(save_file),
                train_data
            )
        ...
        if torch.isinf(loss):
            ...
            if _iter % opt.save_freq == 0 or _iter == 1:
                ...
                for _ in range(5):
                    ...
                    real_data = dataset_val[np.random.permutation(len(dataset_val))[:batch_size_val]]
