...
def main(opt, timestamp = None, trial = None):
    ...
    nlstm = opt.nlstm
    bn = bool(opt.bn)
    cond_disc = False #* force the cond_disc to be False
    disc_kernel_width = 5
    ...
    def disc_training_step(real_data, real_data_p, condition, condition_p, cond_disc=True):
        ...
        fake_data, _ = G(hidden_z, condition, hidden_y)
        fake_data_p, _ = G(hidden_z_p, condition_p, hidden_y_p)
    ...
    def gen_training_step(real_data, real_data_p, condition, condition_p, cond_disc=True):
        ...
        fake_data, fake_condition = G(hidden_z, condition, hidden_y)
        fake_data_p,_ = G(hidden_z_p, condition_p, hidden_y_p)
        #* fake_data.shape: (batch_size, time_steps, Dx)
        #* fake_data refers to system output here
        ...
        loss_rec = opt.rec_lambda * RMSE(fake_condition, condition)

        gen_loss = loss2+loss_rec
    ...
    for _iter in range(1, opt.n_iter):
        ...
        sink_loss, rec_loss = gen_training_step(real_data, real_data_p, condition, condition_p, cond_disc=cond_disc)

        # print and record
        if bool(opt.tb_log):
            writer.add_scalar('training loss', sink_loss+rec_loss, global_step=_iter)
            writer.add_scalar('Sinkhorn training loss', sink_loss, global_step=_iter)
            writer.add_scalar('Reconstruction training loss', rec_loss, global_step=_iter)
            writer.flush()

        if torch.isinf(sink_loss+rec_loss):
            ...
            if _iter % opt.save_freq == 0 or _iter == 1:
                ...
                for _ in range(5):
                    ...
                    condition = x[:,:,:3]
                    samples,_ = G(hidden_z, condition, hidden_y)
                    # compute metric
                ...
                checkpoint['G_loss'].append(sink_loss+rec_loss)
                ...
                if __name__ == "__main__" or opt.training_mode in ['single_run']:
                    print("iter: {}/{}, sink_loss: {:.5f}, rec_loss: {:.5f}, total_loss: {:.5f}, abs_corr: {:.5f}".format(
                        _iter, opt.n_iter, sink_loss.item(), rec_loss.item(),
                        (sink_loss+rec_loss).item(), mean(metrics).item()))
